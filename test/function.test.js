...
test('chains exactly the components given — a two-component path adds no implicit step', async () => {
  let expected = Buffer.from('secret');
  for (const value of ['one', 'two']) {
    expected = crypto.createHmac('sha256', expected).update(value).digest();
  }

  const response = await request(`/${Buffer.from('secret').toString('base64url')}/one/two`);
